...
public abstract class FrameworkServlet extends HttpServletBean implements ApplicationContextAware {
	...
	protected final void initServletBean() throws ServletException {
		...
		try {
			//初始化WebApplicationContext属性，该属性继承自ApplicationContext，该属性是Spring容器上下文
			//FrameworkServlet的作用就是将Servlet与Spring容器进行关联
			this.webApplicationContext = initWebApplicationContext();
			initFrameworkServlet();//该方法的道理与父类initServletBean方法相同，但是DispatcherServlet类并没有重写该方法
		}
	...
	protected WebApplicationContext initWebApplicationContext() {
		...
		//DispatcherServlet有一个以WebApplicationContext 为参数的构造函数，当使用该构造函数时使用这段代码
		...
		//以contextAttribute属性(FrameworkServlet的String类型属性)为key从ServletContext中找到WebApplicationContext,
		//但是contextAttribute一般不会设置。
		if (wac == null) {
		...
		//创建WebApplicationContext并设置根上下文为父上下文然后配置ServletConfig和ServletContext等实例到这个上下文中
		if (wac == null) {
		...
		if (!this.refreshEventReceived) {
			...
			//模板方法WebApplicationContext创建成功后会进行调用，FrameworkServlet没有实现，由子类DispatcherServlet覆盖该方法
			onRefresh(wac);
		}
